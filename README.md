...
| 扩展指令名称      | 子集功能描述             | 状态                    |
| ------- | ---------------- | ----------------------- |
| 多核同步指令      | 可以实现同步、同步清空、同步广播等功能             | :ballot_box_with_check: |
| Cache 指令    | 可以实现清除脏表项、无效表项、按地址无效表项等功能     | :ballot_box_with_check: |
| 算数运算指令   | 可以实现循环右移、低位乘累加、低位乘累减等功能          | :ballot_box_with_check: |
| 存储指令    | 可以实现移位双字加载，符号位扩展字加载、移位双字存储等功能      | :ballot_box_with_check: |  
| 位操作指令    | 可以实现快速找0或1、寄存器连续位提取、字节倒序等功能       | :ballot_box_with_check: |
