...
| 扩展指令子集名称    | 子集功能描述             | 状态                    |
| ------- | ---------------- | ----------------------- |
| 多核同步指令子集    | 可以实现同步、同步清空、同步广播等功能         | :ballot_box_with_check: |
| Cache 指令子集  | 可以实现清除脏表项、无效表项、按地址无效表项等功能     | :ballot_box_with_check: |
| 算数运算指令子集 | 同步清空广播指令 | :ballot_box_with_check: |
| 存储指令子集  | 移位双字加载，移位符号位扩展字加载、移位双字存储等功能     | :ballot_box_with_check: |  
| 位操作指令子集  | 快速找0或1、寄存器连续位提取、字节倒序等功能     | :ballot_box_with_check: |
